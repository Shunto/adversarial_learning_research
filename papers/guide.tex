\documentstyle{kuisthesis}			% 特別研究報告書
...
表の挿入場所は本文の末尾（付録がある場合には，本文と付録の間）とする。
\end{itemize}%}

...
引用文献は本文に続いて（付録があればそのまえに），表の形にまとめる。本文との
対応は[1], [2]のように，[ ]つきの通し番号による。この番号は，該当する文章
...
ソースファイルは以下の形式で作る。
\begin{itemize}\item[]%{
\|\documentstyle{kuisthesis}|\footnote{%
\LATExe をnative modeで使う場合には\CS\documentclass を使
用し，必要に応じて\CS\usepackage を加える。}\quad または\\
\|\documentstyle[master]{kuisthesis}|\footnotemark[1]\quad または\\
\|\documentstyle[master,english]{kuisthesis}|\footnotemark[1]\\
...
\|\documentstyle|（または \|\documentclass|，以下同様）の標準オプションとし
て，以下の3つのものが用意されている。
\begin{itemize}%{
\item
\|master|\\
...
\|\documentstyle|のオプションにしたがって，\|\Large\bf|で印字される。
...
\item[所属学科／専攻]
\|\documentstyle|のオプションと入学年次に応じて，以下のいずれかが\|\large|で
印字される。
\begin{itemize}%{
...
デフォルトでは論文全体の通し番号であるが，前述の\|\documentstyle|のオプショ
...
\footnote{この番号付けは{\tt\string\documentstyle}の{\tt withinsec}オプションと
は無関係である。}。

